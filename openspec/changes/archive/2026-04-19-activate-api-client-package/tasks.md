...
- [x] 8.4 Review the diff once end-to-end before pushing. PR title: `feat(api-client): activate @spacebot/api-client package and migrate interface consumers`. Done — PR #75 opened with this title.
- [x] 8.5 Opened PR #75 with the described sections. Went through `/pr-review-toolkit:review-pr` comprehensive review, three follow-up commits (review findings, CodeQL ReDoS alert #108 fix, zizmor artipacked alert #109 fix), and squash-merged as commit `3cd80c5` on main.

## 9. Post-merge

- [x] 9.1 Archiving via `/openspec-archive-change` right now. Moves this directory to `openspec/changes/archive/2026-04-19-activate-api-client-package/` and merges the capability spec into `openspec/specs/frontend-api-client/spec.md`.
