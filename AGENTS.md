...
**Stack:** Rust (edition 2024), tokio, Rig (v0.35, agentic loop framework), sqlx with `enum DbPool { Sqlite, Postgres }` dual backend (Phase 11.1+; SQLite default, Postgres opt-in via `[database] url`), LanceDB (embedded vector + FTS), redb (embedded key-value).
...
Three supported deployment shapes, each under its own directory at the repo root. Agents making infrastructure changes should know which surface owns what:
