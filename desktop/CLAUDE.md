...
- The root `Cargo.toml` only carries `[workspace] exclude = ["spacedrive"]`; the desktop crate is ignored by Cargo workspace auto-discovery because it sits inside a nested path Cargo doesn't traverse.
