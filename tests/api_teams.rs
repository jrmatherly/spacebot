...
//!   - `unauthenticated_returns_401` (S4 shape: prevents a silent loss of
//!     auth gating from masking as "empty list returned").
//!   - `inactive_teams_are_filtered` — proves the SQL filter, not just
...
async fn unauthenticated_returns_401() {
    // S4 shape: if a router refactor drops the auth layer, an empty teams
    // table would quietly return 200 []. This omits the Authorization header
    // so the missing auth surfaces as 401 explicitly.
