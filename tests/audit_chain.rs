...
async fn first_row_uses_zero_prev_hash() {
    ...
    let row = appender.append(sample_event("u1", AuditAction::AuthSuccess))
        .await.unwrap();
    assert_eq!(row.seq, 1);
    assert_eq!(row.prev_hash, "0".repeat(64),
...
async fn chain_is_linked() {
    let pool = setup_pool().await;
    let appender = AuditAppender::new_for_tests(pool.clone());
    let a = appender.append(sample_event("u1", AuditAction::AuthSuccess)).await.unwrap();
    let b = appender.append(sample_event("u1", AuditAction::ResourceRead)).await.unwrap();
    let c = appender.append(sample_event("u2", AuditAction::AuthFailure)).await.unwrap();

    assert_eq!(b.prev_hash, a.row_hash, "row B.prev_hash must equal row A.row_hash");
    assert_eq!(c.prev_hash, b.row_hash, "row C.prev_hash must equal row B.row_hash");
    assert_eq!(a.seq, 1);
    assert_eq!(b.seq, 2);
    assert_eq!(c.seq, 3);
}
...
async fn row_hash_is_deterministic_for_same_event() {
    ...
    assert_eq!(h1, h2, "canonical bytes must be deterministic for equal inputs");
}
...
async fn concurrent_appends_do_not_corrupt_chain() {
    ...
    for i in 0..20 {
        ...
        handles.push(tokio::spawn(async move {
            a.append(sample_event(&format!("u{i}"), AuditAction::AuthSuccess)).await
        }));
    }
    for h in handles {
    ...
    for i in 1..rows.len() {
        assert_eq!(rows[i].1, rows[i-1].2,
                   "chain break at seq {}: prev_hash != prior row_hash", rows[i].0);
    }
}
...
async fn chain_verify_detects_tamper() {
    let pool = setup_pool().await;
    let appender = AuditAppender::new_for_tests(pool.clone());
    appender.append(sample_event("u1", AuditAction::AuthSuccess)).await.unwrap();
    appender.append(sample_event("u1", AuditAction::ResourceRead)).await.unwrap();
