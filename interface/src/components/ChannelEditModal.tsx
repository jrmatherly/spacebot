			...
			<DialogContent className="max-w-md max-h-[80vh] overflow-y-auto">
					...
					{platform === "signal" && (
						<>
							...
							<p className="mt-3 text-xs text-ink-faint">
								Need help?{" "}
								<a
									href="https://docs.spacebot.sh/signal-setup"
									target="_blank"
									rel="noopener noreferrer"
									className="text-accent hover:underline"
								>
				...
				{configured && (
					<div className="flex flex-col gap-3 border-t border-app-line pt-4">
						...
						{isEditingOrAdding && (
							<div className="flex flex-col gap-3 rounded-md border border-app-line bg-app-dark-box/30 p-3">
								<div>
								...
								{platform === "discord" && (
									<div>
										...
										<p className="mt-1.5 text-xs text-ink-faint">
											Blocks messages entirely — the agent won't see them at
											all. To let the agent read all messages but only respond
											to commands, mentions, or replies, use Mention Only mode
											in Channels settings (cog icon).
										</p>
									</div>
								)}
