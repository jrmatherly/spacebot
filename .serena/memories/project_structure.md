...
│   ├── auth/             # Entra ID JWT validation (landed PR #82, 2026-04-20): errors, context, config, jwks, middleware. Wraps jwt-authorizer 0.15 with RS256 pinned. Siblings `src/auth.rs` (module root) and `src/anthropic_oauth.rs` (formerly `src/auth.rs`, renamed to free the namespace).
