...
├── migrations/           # 55 SQLite migrations: 42 flat per-agent + 13 instance-wide under global/ (2026-02 → 2026-04). Phase 2 (2026-04-21) added 5 instance-wide migrations: users, teams, team_memberships, service_accounts, resource_ownership (the last is a sidecar keyed by `(resource_type, resource_id)` per research §12 Alternative-1). Phase 3 (2026-04-21) added `users_photo` (nullable display_photo_b64 + photo_updated_at for A-19). Phase 4 PR 1 added 0 migrations (pure policy enforcement on existing Phase 2 data model).
