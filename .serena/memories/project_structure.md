...
├── tests/                # 13 integration test files (`spacedrive_client.rs` with wiremock-backed RPC envelope + 401/Bearer round-trip tests; `litellm_proxy.rs` added 2026-04-19 in PR #77 covering [llm.providers.<id>] table form + [[providers]] top-level array form for LLM base_url routing; extended 2026-04-20 in PR #78 with `from_openai_body_parses_litellm_anthropic_cache_tokens` regression guard for `prompt_tokens_details.cached_tokens` parsing; further extended 2026-04-20 in PR #80 with 4 new tests: `update_litellm_provider_writes_block_form_with_api_type_openai_completions`, `update_litellm_provider_preserves_extra_headers_on_empty_submit`, `delete_litellm_preserves_sibling_anthropic_and_parent_providers_table`, `delete_litellm_prunes_empty_parent_providers_table`, `handler_read_chain_resolves_singular_llm_provider_azure`, `update_litellm_preserves_existing_use_bearer_auth_false_on_none_request` — covering UI Update/Delete handlers, the Azure singular→plural `toml_edit::DocumentMut` read-fallback that bypasses the serde alias, and the use_bearer_auth preserve-on-None-request rule)
