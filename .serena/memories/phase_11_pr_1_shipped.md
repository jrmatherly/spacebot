...
> - **Phase 11.2 PR #136 is OPEN as draft (in-flight).** Worktree rebased onto v0.6.3 main; Tasks 1-10 of the per-store dispatch plan committed (14 Postgres migrations + TaskStore/WikiStore/ProjectStore/NotificationStore/AuditAppender on Arc<DbPool>); Tasks 11-15 (auth/repository.rs `&DbPool`, helpers, `ApiState.instance_pool` Arc, handler sweep, legacy migrators) shipped on the rebased branch as of 2026-04-27 evening; Tasks 16-20 still queued. Phase 11.3 / 11.4 still pending. Per-PR plans get written when the predecessor merges (per the writing-plans discipline).
