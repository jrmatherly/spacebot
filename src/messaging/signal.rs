...
impl SignalAdapter {
    ...
    fn process_envelope(&self, envelope: &Envelope) -> Option<(InboundMessage, String)> {
        ...
        let base_conversation_id = if let Some(gid) = group_id {
            ...
            // Canonicalize UUID-backed targets to "signal:uuid:{uuid}" format
            if let Some(uuid) = envelope.source_uuid.as_deref() {
...
mod tests {
    ...
    fn build_metadata_dm() {
        ...
        assert!(!metadata.contains_key("signal_group_id"));
    ...
    fn process_envelope_conversation_id_dm() {
        ...
        // Should use UUID when available, in canonical "signal:uuid:{uuid}" format.
