...
- `docs/design-docs/postgres-migration.md` — Phase 11 dual-backend architecture: `enum DbPool` over native typed sqlx pools, `DialectAdapter` companion trait, `[database]` config block, `DatabaseUrl` newtype validated at TOML deserialize. PR 11.1 (foundation) and PR 11.2 (instance-tier dispatch + 5 stores + testcontainers acceptance test) shipped; per-agent dispatch lands in PR 11.3, K8s deployment in PR 11.4.
