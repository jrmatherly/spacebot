...
- **Stderr plumbing.** The sidecar's stderr today goes to Tauri's stdout capture. During the PR #60 review cycle, finding P-1 raised uncertainty about whether the frontend sees readiness signals emitted by the daemon; the default `tracing-subscriber` writer is `io::stdout`, so the readiness path should work, but no end-to-end test confirms it. A dedicated stderr channel to Tauri, and an explicit test that asserts "daemon ready" propagates to the UI within 5 seconds of spawn, would harden the signal. Original review notes are preserved as historical record, gitignored.
