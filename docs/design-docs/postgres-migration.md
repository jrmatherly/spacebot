...
> Superseded by the planned `phase-11-postgres-backend.md` track. Created during Phase 10 so SOC 2 evidence can reference it as the home for any future Postgres-related decisions.
...
SOC 2 controls for the K8s tier rely on volume-level encryption at the storage layer (Talos system-disk encryption, verified in the cluster repo), NOT on Postgres. Phase 10 ships SOC 2 evidence against the current SQLite-on-PVC posture; Postgres is a performance and operational upgrade for later.
