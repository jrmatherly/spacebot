---
title: Configuration
description: How config.toml loads, resolves, and hot-reloads. Field-by-field reference lives at /docs/config-reference.
---

# Configuration

Spacebot looks for `config.toml` at `~/.spacebot/config.toml` (or `$SPACEBOT_DIR/config.toml`). If no config file exists, it falls back to environment variables. This page covers **how** config works. For the exhaustive **field-by-field reference** (every option, type, and default drawn from the Rust schema), see [Config Reference](/docs/config-reference).
...
```toml
[llm.providers.custom_anthropic]
api_type = "anthropic"
base_url = "https://api.anthropic.com"
...
Optional. When present with `enabled = true`, the daemon validates incoming `Authorization: Bearer <jwt>` headers as Microsoft Entra ID v2 JWTs via a cached JWKS. Replaces the static `auth_token` branch for that run. The two branches are mutually exclusive at request time.

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `enabled` | bool | false | Master switch for the Entra branch |
| `tenant_id` | string | **required** | Entra tenant GUID. Supports `env:ENTRA_TENANT_ID` |
| `audience` | string | **required** | Expected `aud` claim. For v2 tokens this is the **client ID GUID** of the Web API registration (not the `api://...` URI). Requires `accessTokenAcceptedVersion: 2` in the manifest |
| `allowed_scopes` | string[] | `[]` | Required scopes for delegated (user) tokens. Empty = any scope or app-only |
| `jwks_cache_ttl_secs` | integer | 3600 | Max JWKS cache age. Microsoft recommends at most 24h |
| `clock_skew_leeway_secs` | integer | 60 | Tolerance on `exp` and `nbf`. Must not exceed 300 |
| `group_cache_ttl_secs` | integer | 300 | Graph group-membership cache TTL (Phase 3 consumes) |
| `spa_client_id` | string | `""` | SPA app-registration client ID GUID. Returned via `/api/auth/config` to the browser (never a secret) |
| `spa_scopes` | string[] | `[]` | Scopes the SPA requests during sign-in |
| `mock_mode` | bool | false | **Dev only.** Bypass signature validation. Rejected at daemon startup. Use integration-test helpers instead |
...
See `docs/design-docs/entra-app-registrations.md` for app-registration setup.
