...
| Spacebot-owned Spacedrive policy files | `spacedrive/README.md`, `spacedrive/SECURITY.md`, `spacedrive/CONTRIBUTING.md`, `spacedrive/CODE_OF_CONDUCT.md`, `spacedrive/AGENTS.md` — these 5 root-level files are IN scope as Spacebot-owned policy documents for the vendored subtree (banners, clone URLs, issue/PR routing pointed at `jrmatherly/spacebot`). The rest of `spacedrive/` (crates/, core/, apps/, docs/, extensions/, adapters/, schemas/) is deferred to its own review cycle — it's our code per the 2026-04-16 self-reliance decision, but the docs-audit skill's remit is user-facing and internal-ops documentation, not imported source file prose. |
...
| Custom agents | `.claude/agents/*.md` — 14 files (migration-writer, security-reviewer, dep-spec-auditor, plus 11 Phase-rollout auditors: authz-gate-conformance, code-doc-sync-auditor, dependabot-triager, fmt-drift-detector, integration-test-coverage-auditor, openapi-typegen-verifier, phase-plan-auditor, skill-drift-auditor, spaceui-token-conformance, tracing-log-contract, write-path-sweep-verifier). dep-spec-auditor added 2026-04-26 (commit `0367bad`) — read-only manifest-vs-lockfile-vs-latest drift auditor across all 7 workspaces. |
...
| `spacedrive/` (241 tracked files) **except** the five policy files listed in Tier 1 above | Spacebot-owned fork source (`crates/`, `core/`, `apps/`, `docs/`, `extensions/`, `adapters/`, `schemas/`, `whitepaper/`). Our code per the 2026-04-16 self-reliance decision, maintained on our own schedule. Out of scope for THIS skill because the docs-audit remit is user-facing + internal-ops docs; imported source-file prose is a separate, lower-priority review. | Future dedicated spacedrive-fork review |
