...
> ⚠️ **Reference for external consumers — not this fork's workflow.** This file documents how an outside project would consume SpaceUI if the packages were published to npm (upstream's model). In this repository, SpaceUI is NOT published externally. `interface/` consumes the packages via the bun workspace protocol (see [`README.md`](./README.md) and [`CONTRIBUTING.md`](./CONTRIBUTING.md)). This guide is kept as canonical documentation of the patterns (Vite aliases, Tailwind `@source` directives, React deduplication, Next.js + React Native variants) in case a future external consumer or a publishing decision revives it. Claims in this file about npm publication, release CI, and the `@spacedrive` npm scope reflect upstream's workflow, not this fork's.
