...
- **`[api.auth.entra]` config + Entra JWT validation middleware.** New `src/auth/` module (`errors`, `context`, `config`, `jwks`, `middleware`) validates Microsoft Entra ID v2 JWTs via a JWKS-cached `jwt-authorizer` 0.15 wrapper. `EntraValidator` distinguishes delegated (`scp`-bearing) and application (`roles`-bearing) tokens, pins `Algorithm::RS256`, and enforces required-scope for users plus required-role for service-principals. `AuthContext` now travels through request extensions from either the static-token or Entra branch, so downstream handlers can uniformly extract `{ tid, oid, principal_type, roles, groups, groups_overage, display_email, display_name }` (no authz yet; Phase 4 adds that). `ApiState.entra_auth` is an `ArcSwap<Option<Arc<EntraValidator>>>` following the `task_store` / `wiki_store` setter pattern. Branch selection happens once in `start_http_server`; the two middlewares are never both attached. `[api.auth.entra].mock_mode = true` is rejected at daemon startup. Documented at `docs/content/docs/(configuration)/config.mdx` and `docs/design-docs/entra-app-registrations.md`. Tests: 5 new unit tests (`auth::context::*`, `auth::config::*`) plus 4 new Wiremock-backed integration tests in `tests/entra_jwt_middleware.rs` covering happy path, wrong-signature rejection, and empty-token rejection. `tests/support/mock_entra.rs` helper uses pure-Rust `rsa` 0.9 (A-07 avoids the libssl system dep) with `rsa::rand_core::OsRng` to bridge the `rand 0.10` / `rsa 0.9` MSRV gap. Precursor: `src/auth.rs` (Anthropic OAuth) renamed to `src/anthropic_oauth.rs` to free the `auth` namespace; 10 call sites migrated with git rename detection preserving history.
