...
- **`ci.yml` Test-job cache eviction loop broken.** The Test job had never successfully saved a cache (verified via `gh api repos/.../actions/caches` — zero Test-job entries in repo history). Root cause: GitHub's 10 GB per-repo cache cap combined with `Swatinem/rust-cache`'s per-job auto-keys produced two ~1.3 GB duplicate caches for the same dep graph (one for `check`, one for `test`), and the Test slot was the LRU eviction victim on every run. Every CI run re-paid the full cold-cache compile cost (~25 min for ~1000 dep crates), which exceeded the 20 min `timeout-minutes`. Four structural changes land together: (1) `check` and `test` now share a cache slot via `shared-key: ci-rust-build`; (2) `test` now `needs: check` so the cache is pre-warmed before test tries to restore (also skips test when check fails — correctness bonus); (3) timeouts bumped (Test 20 → 40 min, OpenAPI Typegen Drift 15 → 30 min) for cold-cache runway; (4) `check-typegen` gets its own explicit `shared-key: ci-rust-typegen` (compiles the `openapi-spec` bin target, different fingerprints than lib tests). Companion housekeeping via gh api: three stale `-ee1c38bd`-suffix caches (4.2 GB) deleted because their Cargo.lock hash has changed and they could never restore again. Total repo cache dropped from 10.31 GB to 6.10 GB, well under the cap.
