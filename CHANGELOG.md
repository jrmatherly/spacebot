...
- **SOC 2 evidence package (Phase 10, FINAL phase of the Entra rollout).** Closes the 10-phase Entra ID rollout. New `GET /api/admin/access-review` (CC6.7 quarterly evidence; CSV default + JSON via `?format=json`; per-principal identity, status, last_seen_at, and team membership; admin-gated with `tracing::warn!` on role denial; emits `AdminRead` audit event). New `GET /api/admin/orphans` (on-demand cross-agent ownership-drift report; admin-gated with `AdminRead` emission per sweep). New `src/admin.rs` module exposing `sweep_orphans(instance_pool, agent_db_paths)` which walks per-agent SQLite files plus the instance-level `resource_ownership` table in both directions: `MissingOwnership` (agent has the resource, no instance row) and `StaleOwnership` (instance row, vanished agent or resource). Race-tolerant per IMPORTANT-7 #5: a missing or partially-initialized `spacebot.db` is logged via `tracing::warn!` and skipped, not crashed. `discover_agent_db_paths()` scans `$SPACEBOT_DIR/agents/*/data/spacebot.db` and gracefully handles missing env. Existing Phase 5 chain-verify endpoint at `GET /api/admin/audit/verify` continues to ship the SOC 2 CC7.2 evidence; Phase 10 references it without duplication. **JWKS rotation correctness fix** at `src/auth/jwks.rs`: jwt-authorizer 0.15's default `Refresh { refresh_interval: 600s, strategy: KeyNotFound }` requires the load time to exceed `refresh_interval` before an unknown-`kid` triggers a JWKS refetch. Inside that window, a token signed with a freshly-rotated Entra key returned 401 `InvalidKid` for up to 10 minutes after the daemon's last successful fetch. Replaced with `Refresh { refresh_interval: ZERO, retry_interval: 10s }` so unknown-kid unconditionally refetches; `retry_interval` still rate-limits hits to a known-failing JWKS endpoint. Regression-guarded by `tests/jwks_rotation.rs::validator_refetches_jwks_on_unknown_kid`, which uses `MockTenant::rotate_keys()` (new wiremock-backed key-rotation helper, pure-Rust `rsa` per A-07) to mint pre- and post-rotation tokens and assert both validate. **CODEOWNERS update**: `.github/CODEOWNERS` declares the security-sensitive paths under SOC 2 CC8.1 change-management policy (auth/audit/secrets/admin handlers/global migrations/security docs); branch protection enforces the required-reviewer count separately. **Six new SOC 2 docs**: `docs/design-docs/entra-architecture-diagram.md` (Mermaid sequence diagram of end-to-end auth flow, defense-in-depth layers, token-storage table covering all four surfaces post-Phase-9); `docs/runbooks/entra-auth-incidents.md` (CC7.4 triggers + thresholds + response playbooks for failed-auth spike, JWKS anomaly, audit-chain tamper, group-membership anomaly, orphaned-ownership accumulation); `docs/runbooks/entra-change-management.md` (CC8.1 two-person review, pre-prod tenant procedure, release procedure, credential rotation cadence); `docs/security/data-classification.md` (CC6.1 Public/Internal/Confidential/Restricted tiers, per-table mapping, encryption-at-rest paragraph honoring IMPORTANT-10: K8s tier relies on Talos `systemDiskEncryption` verified in the cluster repo); `docs/security/third-party-assessments/microsoft-entra.md` (reliance scope, outage scenarios, compensating controls); `docs/security/pentest-scope.md` (auth middleware, admin endpoints, audit chain, Tauri loopback, CLI device-code phishing); `docs/security/entra-soc2-evidence-index.md` (top-level register of all 10 artifacts with owner + cadence + curl recipes for auditors); `docs/design-docs/postgres-migration.md` (roadmap stub, not blocking SOC 2). New `scripts/generate-access-matrix.sh` regenerates `docs/design-docs/entra-access-matrix.md` from a running daemon's access-review endpoint (quarterly evidence-collection cadence). OpenAPI schema regenerated for the two new endpoints; new `OrphansResponse` and `OrphanReport` schemas exposed via `@spacebot/api-client`. Coverage: 3 new tests in `tests/api_admin_access_review.rs` (non-admin 403, admin CSV with header + content, admin JSON with teams array + null-row narrowing); 3 new tests in `tests/orphan_sweep.rs` (MissingOwnership detection, partially-initialized agent tolerance per IMPORTANT-7 #6, discover_agent_db_paths empty-on-no-env); 1 new test in `tests/jwks_rotation.rs` (unknown-kid post-rotation refetch). All 19 existing `tests/entra_jwt_middleware.rs` tests still pass — JWKS-refetch behavior change is additive, not a regression to the prior contract. Plan amendments applied pre-kickoff: `src/api/admin/` nested layout flattened to `src/api/admin_*.rs` (matches Phase 7/9 convention), `sweep_orphans` test rewritten to match `Vec<PathBuf>` impl signature with tempdir-backed agent DB setup (A-21 real enumeration, not hand-curated list), `spacebot::api::ApiState::new_test_state_with_mock_entra` import path corrected (4 sites), `spacebot entra admin claim-resource` namespace fix (post-D-26), explicit `pub mod admin;` wiring step added to Task 10.5, MockTenant-vs-mint_mock_token rationale documented in Task 10.4, cargo budget recounted from 6 to 4-5 invocations, MSAL.js `cacheLocation: "memoryStorage"` corrected per A-16, Phase 11 wording (Phase 10 is the final phase of the Entra ID **auth** rollout; Phase 11 is the separate Postgres-backend track). Deferred follow-ups (not blocking Phase 10): automated weekly orphan-sweep schedule (IMPORTANT-7 #1, requires a new instance-level maintenance subsystem outside per-agent cortex), per-agent-DB AdminRead emission (IMPORTANT-7 #2), `auto_delete_stale` config flag + AdminWrite-per-deletion (IMPORTANT-7 #3 + #4). The orphan sweep currently runs on-demand via the admin endpoint, which itself satisfies the dry-run-by-default requirement.
