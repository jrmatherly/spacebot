...
- **`spacebot_auth_failures_total{branch, reason}` Prometheus counter.** Auth-middleware rejection rate is now observable. `branch` is `static_token` (today) or `entra_jwt` (Phase 1+). `reason` is machine-readable; the static path emits `bearer_mismatch`. Feature-gated (`metrics`).
- **`test-support` Cargo feature.** Opens `#[doc(hidden)]` scaffolding (`ApiState::new_for_tests`, `api::server::test_support::build_test_router`) to integration tests under `tests/*.rs`. Unit tests in `src/**` see this free via `cfg(test)`. Not intended for downstream consumers.
